...
Editor.prototype.inline = function ( cell, fieldName, opts )
{
	...
	$.each( editFields, function ( i, edit ) {
		if ( countOuter > 0 ) {
...
var __dtCellSelector = function ( out, dt, identifier, allFields, idFn, forceFields )
{
	dt.cells( identifier ).indexes().each( function ( idx ) {
		...
		// If already flagged for editing, add our field (rows automatically
		// edit all fields)
		if ( out[ idSrc ] && out[ idSrc ].type !== 'row' ) {
			$.each( fields, function ( name, field ) {
				if ( ! out[ idSrc ].fields[ name ] ) {
					out[ idSrc ].fields[ name ] = field;
					out[ idSrc ].attach.push( cell.node() );
				}
			} );
		}
		else if ( ! out[ idSrc ] ) {
